    ...
    for block_id in range(base_block, base_block+block_offset+1):
        ...
        if block:
            ...
            try:
                ...
                print("success: {}".format(block_id))
            except Exception as e:
                print("{} Block Insertion Failed {}".format(block_id, e))
                logging.info("{} Block Insertion Failed {}".format(block_id, e))
            for transaction in block["transactions"]:
                transaction_id = transaction.hex()
                transaction_details = get_transaction_info(w3, transaction.hex())
                if transaction_details:
                    transaction_details["value_wei"] = str(transaction_details["value"])
                    transaction_details["value"] = transaction_details["value"] / pow(10, 18)
                    transaction_details["gas_wei"] = str(transaction_details["gas"])
                    transaction_details["gas"] = transaction_details["gas"] / pow(10, 18)
                    transaction_details["gasPrice_wei"] = str(transaction_details["gasPrice"])
                    transaction_details["gasPrice"] = transaction_details["gasPrice"] / pow(10, 18)
                    try:
                        transactions.insert_one(transaction_details)
                        print("success: {}/{}".format(block_id, transaction_id))
                        logging.info("success: {}/{}".format(block_id, transaction_id))
                    except Exception as e:
                        print("{} Transaction Insertion Failed {}".format(transaction_id, e))
                        logging.info("{} Transaction Insertion Failed {}".format(transaction_id, e))

                else:
                    print("Transaction {}  {}".format(transaction_id, "Failed"))
                    logging.info("Transaction {}  {}".format(transaction_id, "Failed"))
                    continue
        else:
